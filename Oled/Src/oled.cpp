...
void OLED_SendCmd(uint8_t cmd) {
  ...
  sendBuffer[0] = 0x00;
  sendBuffer[1] = cmd;
  HAL_I2C_Master_Transmit(&i2c_Port, OLED_ADDRESS, sendBuffer, 2, HAL_MAX_DELAY);
}
...
void OLED_Send(uint8_t *data, uint8_t len) {
  HAL_I2C_Master_Transmit(&i2c_Port, OLED_ADDRESS, data, len, HAL_MAX_DELAY);
}
...
void OLED_Init() {
  ...
  OLED_SendCmd(0x14);
  OLED_SendCmd(0x20);     //设置内存地址
  OLED_SendCmd(0x02);
  OLED_SendCmd(0xA1);     //设置段重定义
  OLED_SendCmd(0xC8);     //设置 COM 扫描方向
  OLED_SendCmd(0xDA);     //设置 COM 硬件引脚配置
  OLED_SendCmd(0x12);
  OLED_SendCmd(0x81);     //对比度设置
  OLED_SendCmd(0xCF);
  OLED_SendCmd(0xD9);     //设置充电周期
  OLED_SendCmd(0xF1);
  OLED_SendCmd(0xDB);     //设置 VCOMH 电压倍率
  OLED_SendCmd(0x40);
...
void OLED_Test() {
  ...
  HAL_I2C_Master_Transmit(&i2c_Port, OLED_ADDRESS, sendBuffer, sizeof(sendBuffer), HAL_MAX_DELAY);
}
...
extern "C" {
    void OLED_SendCmd_(uint8_t cmd) { OLED_SendCmd(cmd); }
    void OLED_Send_(uint8_t *data, uint8_t len) { OLED_Send(data, len); }
    void OLED_Init_(void) { OLED_Init(); }
    void OLED_Test_(void) { OLED_Test(); }

    void OLED_DisPlay_On_(void) { OLED_DisPlay_On(); }
    void OLED_DisPlay_Off_(void) { OLED_DisPlay_Off(); }

    void OLED_NewFrame_(void) { OLED_NewFrame(); }
    void OLED_ShowFrame_(void) { OLED_ShowFrame(); }

    void OLED_DrawLine_(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, OLED_ColorMode color) { OLED_DrawLine(x1, y1, x2, y2, color); }
    void OLED_DrawCircle_(uint8_t x, uint8_t y, uint8_t r, OLED_ColorMode color) { OLED_DrawCircle(x, y, r, color); }
    
    void OLED_PrintASCIIChar_(uint8_t x, uint8_t y, char c, const Font *font, OLED_ColorMode color) { OLED_PrintASCIIChar(x, y, c, (const ASCIIFont *)font, color); }
    void OLED_PrintString_(uint8_t x, uint8_t y, char *str, const Font *font, OLED_ColorMode color) { OLED_PrintString(x, y, str, font, color); }
